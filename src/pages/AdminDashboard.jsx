...
function OverviewSection() {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      ...
      <div style={s.grid(3)}>
        <StatCard label="Total Cars" value={fmtNum(stats?.totalCars)} sub={`${stats?.pendingCars} pending approval`} color={C.accent} icon="🚗" />
    ...
    try {
      ...
      if (response.message === "User created successfully") {
        ...
        setForm({ 
          ...
          mobile_no: "", branch: "", dob: null, marrieddate: null, address: "", permissions: [] 
        });
        load();
      } else {
...
function BranchesSection({ toast }) {
  ...
  const handleCreate = async () => {
    try {
      await apiPost("/branch/create-branch", form);
      toast("Branch created!", "success");
      setShowCreate(false); setForm({ name:"",address:"",city:"",state:"",zipCode:"",phone:"",email:"",branchHeadId:"" });
      load();
  ...
  const openEdit = (b) => {
    setEditBranch(b);
    setForm({ name:b.name, address:"", city:b.city, state:"", zipCode:"", phone:"", email:"" });
  };

  const BranchForm = ({ onSubmit, submitLabel }) => (
    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      {[["Branch Name","name"],["Address","address"],["City","city"],["State","state"],["Zip Code","zipCode"],["Phone","phone"],["Email","email"],["Branch Head ID (optional)","branchHeadId"]].map(([label,key])=>(
        <div key={key}>
      ...
      <div style={{ display:"flex", gap:10, marginTop:8 }}>
        ...
        <button style={s.btn(C.muted,true)} onClick={()=>{setShowCreate(false);setEditBranch(null);}}>Cancel</button>
      </div>
    </div>
  );
      ...
      {showCreate && (
        <div style={s.modal} onClick={()=>setShowCreate(false)}>
          <div style={s.modalBox} onClick={e=>e.stopPropagation()}>
            ...
            <BranchForm onSubmit={handleCreate} submitLabel="Create Branch" />
          </div>
        </div>
      )}
      ...
      {editBranch && (
        <div style={s.modal} onClick={()=>setEditBranch(null)}>
          <div style={s.modalBox} onClick={e=>e.stopPropagation()}>
            ...
            <BranchForm onSubmit={handleUpdate} submitLabel="Save Changes" />
          </div>
        </div>
      )}
    </div>
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (filter === "pending") {
        ...
        console.log("Pending cars response:", res);
